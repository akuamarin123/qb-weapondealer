...
QBCore.Functions.CreateCallback('qb-weapondealer:server:GetPlayerStats', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return cb(nil) end

    MySQL.query('SELECT * FROM weapon_dealers WHERE citizenid = ?', {Player.PlayerData.citizenid}, function(result)
        if result[1] then
            local stats = {
                reputation = result[1].reputation or 0,
                total_sales = result[1].total_sales or 0,
                workshop_level = result[1].workshop_level or 1
            }

            -- İtibar seviyesini hesapla
            local currentLevel = 1
            for level, data in pairs(Config.ReputationLevels) do
                if stats.reputation >= data.minRep and stats.reputation <= data.maxRep then
                    currentLevel = level
                    stats.level_label = data.label
                    stats.next_level_rep = data.maxRep
                    break
                end
            end
            ...
            cb({
                ...
                workshop_level = 1,
                current_level = 1,
                level_label = Config.ReputationLevels[1].label,
                next_level_rep = Config.ReputationLevels[1].maxRep
            })
        end
    end)
end)

-- Satılabilir eşyaları getirme callback'i
QBCore.Functions.CreateCallback('qb-weapondealer:server:GetSellableItems', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    for _, item in pairs(inventory) do
        -- Silah veya mermi kontrolü
        if string.find(item.name, "weapon_") or string.find(item.name, "_ammo") then
            local itemType = string.gsub(item.name, "weapon_", "")
            items[#items + 1] = {
                name = item.name,
                label = item.label,
                type = itemType,
...
-- Eşya satma
RegisterNetEvent('qb-weapondealer:server:SellItem', function(itemName, amount, price)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if Player.Functions.RemoveItem(itemName, amount) then
        -- Para ekleme
        Player.Functions.AddMoney('cash', price * amount, "weapon-sale")
        
        -- İtibar puanı ekleme (her satışta +5 puan)
        UpdateReputation(Player.PlayerData.citizenid, 5)
        
        -- Toplam satış sayısını güncelleme
        MySQL.query('UPDATE weapon_dealers SET total_sales = total_sales + ? WHERE citizenid = ?', 
            {amount, Player.PlayerData.citizenid})

        -- Bildirim
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[itemName], "remove")
        TriggerClientEvent('QBCore:Notify', src, string.format('Satış başarılı! Kazanç: $%s', price * amount), 'success')

        -- Polis bildirimi için şans hesaplama
        local notifyChance = 0
        if string.find(itemName, "weapon_") then
            notifyChance = 40 -- Silah satışında %40 bildirim şansı
        else
            notifyChance = 20 -- Mermi satışında %20 bildirim şansı
        end

        -- Polis bildirimi
        if math.random(1, 100) <= notifyChance then
            -- Oyuncunun konumunu al
            local coords = GetEntityCoords(GetPlayerPed(src))
            
            -- Tüm polislere bildirim gönder
            local players = QBCore.Functions.GetQBPlayers()
            for _, v in pairs(players) do
                if v.PlayerData.job.name == "police" and v.PlayerData.job.onduty then
                    -- Bildirim başlığını belirle
                    local title = string.find(itemName, "weapon_") and "Şüpheli Silah Satışı" or "Şüpheli Mermi Satışı"
                    
                    TriggerClientEvent('police:client:PoliceAlert', v.PlayerData.source, {
                        title = title,
                        coords = {x = coords.x, y = coords.y, z = coords.z},
                        description = "Bölgede şüpheli satış aktivitesi tespit edildi!"
                    })
                end
            end

            -- Satıcıya bildirim
            TriggerClientEvent('QBCore:Notify', src, 'Dikkat! Satış şüpheli görüldü, polis bilgilendirilmiş olabilir!', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', src, 'Yeterli eşya yok!', 'error')
    end
end) 